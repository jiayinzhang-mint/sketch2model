...
class BaseOptions:
    ...
    def initialize(self, parser):
        ...
        # basic parameters
        parser.add_argument('--name', type=str, default='airplane_pretrained', required=True,
        ...
        parser.add_argument('--class_id', type=str, default='02691156', required=True)

        # model parameters
        parser.add_argument('--model', type=str, default='view_disentangle', choices=['view_disentangle'],
                            help='chooses which model to use.')
        parser.add_argument('--dim_in', type=int, default=3,
                            help='number of input channels for image feature extractor')
        ...
        parser.add_argument('--template_path', type=str, default='templates/sphere_642.obj',
                            help='path to the base mesh')

        # dataset parameters
        parser.add_argument('--dataset_mode', type=str, default='shapenet',
                            choices=['shapenet', 'shapenet_sketch', 'inference'],
        ...
        parser.add_argument('--suffix', default='', type=str,
                            help='customized suffix: opt.name = opt.name + suffix: e.g., {model}_{batch_size}')
    ...
    def gather_options(self):
        ...
        model_option_setter = options.get_option_setter(
            find_class_using_name(f"models.{model_name}_model", model_name, 'model', BaseModel))
        parser = model_option_setter(parser)

        # modify dataset-related parser options
        dataset_name = opt.dataset_mode
        dataset_option_setter = options.get_option_setter(
            find_class_using_name(f"data.{dataset_name}_dataset", dataset_name, 'dataset', BaseDataset))
    ...
    def parse(self):
        ...
        opt.is_test = self.is_test
        opt.is_infer = self.is_infer
